...
public:
	...
	bool stop; // is set to true if the program is bound to be closed.
	int fontSmallSize; // size of the large font.
	int fontLargeSize; // size of the small font.
	virtual ~Engine();
	virtual int newWindow(int width, int height) = 0; //create a new SDL window. Parameters are for it's size.
	virtual void loadFonts() = 0; // Load all fonts in the right size.
	...
	virtual void increaseResolution() = 0; // Increase the window resolution.
	virtual void decreaseResolution() = 0; // decrease the window resolution.
	...
	std::vector<float> transform(float x, float y, float w, float h); // Transform the coordinates from the game to the pixel values needed for rendering.
	std::vector<float> transformSize(float w, float h); // Transform the pixel values of the height and width of a texture to the game's coordinate system.
};
